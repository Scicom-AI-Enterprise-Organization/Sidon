...
from .model import add_cond_adapters_all_layers
...
class DialogueFeaturePredictorLightningModule(LightningModule):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.teacher_ssl_model = transformers.Wav2Vec2BertModel.from_pretrained(
            cfg.ssl_model_name, num_hidden_layers=16
        ).eval()
        self.student_ssl_model = add_cond_adapters_all_layers(
            self.student_ssl_model,
            d_cond=128,
            hidden=256,
        )
        self.conditoning_emb = torch.nn.Embedding(2,128)
        ...
        #     param.requires_grad = False
        # layers = self.student_ssl_model.encoder.layers
        # for i in range(len(layers)):
        #     if hasattr(layers[i],'cond_adapter'):
        #         for p in layers[i].cond_adapter.parameters():
    ...
    def step(self, batch, batch_idx: int, stage: str = "train") -> torch.Tensor:
        noisy_ssl_inputs = batch["noisy_mixture_ssl_inputs"]
        clean_ssl_inputs_0 = batch["clean_ssl_inputs_0"]
        clean_ssl_inputs_1 = batch["clean_ssl_inputs_1"]
        batch_size = noisy_ssl_inputs['input_features'].shape[0]
        targets = []
        preds = []
        for speaker_idx, clean_ssl_inputs in enumerate([clean_ssl_inputs_0,clean_ssl_inputs_1]):
            speaker_idx = torch.tensor(speaker_idx,device=self.device).repeat(batch_size)
            speaker_cond = self.conditoning_emb(speaker_idx)
            with torch.inference_mode():
                teacher_features = self.teacher_ssl_model(
                    **clean_ssl_inputs
                ).last_hidden_state
                targets.append(teacher_features)
            student_features = self.student_ssl_model(**noisy_ssl_inputs,cond=speaker_cond).last_hidden_state
            preds.append(student_features)
        # Compute loss in float32 and sanitize NaNs/Infs to prevent instability under AMP
        p = torch.stack(preds).float()
        t = torch.stack(targets).float()
        if torch.isnan(t).any():
            raise ValueError("target has nan")
        p = torch.nan_to_num(p)
        loss_a = self.ssl_model_criterion(p, t)
        loss_b = self.ssl_model_criterion(p, t.flip(0))  # swap speaker assignment
        ssl_loss = torch.minimum(loss_a, loss_b)
...
class DialogueSidonLightningModule(LightningModule):
    ...
    def step(
        ...
        wavs = batch["input_wav"][:,0,:] # only use first channel
        ...
        with torch.no_grad():
            if self.cfg.pretraining:
                clean_ssl_inputs = batch["clean_ssl_inputs_0"]
...
class CleanSepFeaturePredictorLightningModule(DialogueFeaturePredictorLightningModule):
    def step(self, batch, batch_idx: int, stage: str = "train") -> torch.Tensor:
        noisy_ssl_inputs = batch["clean_mixture_ssl_inputs"]
        clean_ssl_inputs_0 = batch["clean_ssl_inputs_0"]
        clean_ssl_inputs_1 = batch["clean_ssl_inputs_1"]
        batch_size = noisy_ssl_inputs['input_features'].shape[0]
        targets = []
...
class E2EDialogueSidon(LightningModule):
    """End to end dialogue sidon"""
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.save_hyperparameters()
        self.student_ssl_model = transformers.Wav2Vec2BertModel.from_pretrained(
            cfg.ssl_model_name, num_hidden_layers=cfg.get("student_layer",8),layerdrop=0.0,
        )
        self.teacher_ssl_model = transformers.Wav2Vec2BertModel.from_pretrained(
            cfg.ssl_model_name, num_hidden_layers=cfg.get("student_layer",8),layerdrop=0.0,
        ).eval()
    ...
    def feature_predictor_step(self,batch,batch_idx:int,stage:str = 'train'):
        noisy_ssl_inputs = batch["noisy_mixture_ssl_inputs"]
        clean_ssl_inputs_0 = batch["clean_ssl_inputs_0"]
        clean_ssl_inputs_1 = batch["clean_ssl_inputs_1"]
        batch_size = noisy_ssl_inputs['input_features'].shape[0]
        ...
        if torch.isnan(t).any():
            raise ValueError("target has nan")
        p = torch.nan_to_num(p)
        loss_a = self.ssl_model_criterion.forward(p, t)
        loss_b = self.ssl_model_criterion.forward(p, t.flip(0))  # swap speaker assignment
        ssl_loss = torch.minimum(loss_a, loss_b)
        if loss_b < loss_a:
            p = p.flip(0)
        p=p.permute(1,0,2,3)
    ...
    def step(
        ...
        channel_idx = random.randint(0,1)
        wavs = batch["input_wav"][:,channel_idx,:] # only use first channel
        wavs = wavs / wavs.abs().max()
        batch_size = wavs.shape[0]
        opt_g, opt_d = self.optimizers()  # type: ignore
        sch_g, sch_d = self.lr_schedulers()  # type: ignore

        ssl_loss, predicted_features, target_features = self.feature_predictor_step(batch,batch_idx,stage=stage)
        decoder_input = target_features[:,channel_idx]

        input_features = decoder_input.transpose(1, 2)
        predicted_clean_wavs = self.decoder.forward(input_features)
        if abs(predicted_clean_wavs.shape[-1] - wavs.shape[-1]) > 480:
            raise ValueError("Predicted waveform length deviates too much from target")
        min_length = min(predicted_clean_wavs.shape[-1], wavs.shape[-1])
        ...
        self.log(f"{stage}/total_loss", total_loss,prog_bar=True)
    ...
    def configure_optimizers(self):  # type: ignore
        opt_g = torch.optim.AdamW(
            [{ 'params': self.student_ssl_model.parameters(), 'lr': 2e-5, 'weight_decay': 0.01},
                {'params': self.conditoning_emb.parameters(), 'lr': 2e-5, 'weight_decay': 0.01},
                {'params': self.decoder.parameters(), 'lr': 1e-4, 'weight_decay': 0.01},
            ],
            lr=self.cfg.optim.lr,
            weight_decay=self.cfg.optim.weight_decay,
            betas=(0.8, 0.98),
